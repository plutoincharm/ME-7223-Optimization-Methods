...
x1Label = linspace(-50,50,100);
x2Label = linspace(-50,50,100);
[V,D] = meshgrid(x1Label,x2Label);
f2 =  (V+2*D-7).^2+(2*V +D-5).^2;  % objective Function 
figure()
...
for i=1:5 
    ...
    f = (X(1) + 2*X(2)-7)^2 + (2*X(1)+X(2)-5);%initial value
    S = S1;
  
    X2 = X - e*S;
    
    f2 = (X(1) + 2*X(2)-7)^2 + (2*X(1)+X(2)-5);
    if f2<f
        S = -S;
    end
    fun = @(lambda) (X(1) + lambda*S(1)+ 2*(X(2) + lambda*S(2))+7)^2  + (2*(X(1) + lambda*S(1)) + (X(2) + lambda*S(2))-5)^2;
    l = fminbnd(fun,-2,2);
